#!/bin/bash

#SBATCH --output=job_sim.out
#SBATCH --time=2:59:00
#SBATCH --nodes=2
#SBATCH --ntasks-per-node=192
#SBATCH --mem=0
#SBATCH --job-name=para_test
#SBATCH --account=def-goluskin


# All the exports were by recommendation of DRAC support when troubleshooting an issue
################################################################################
# Simulation parameters
export START_TIME=0
# Rayleigh number
export RA=7
# Exponent of 10 for Prandtl number. (ie if Pr=1, PR_EXP=0)
export PR=-2
# Vertical resolution
export RES=96
# Timestep- if using fixed timestep this matters. Otherwise just leave
# sufficiently small that the simulation won't blow up in 25 iterations
export DT=0.005
# Dimensionless time to run the simulation for
export SIM_TIME=10
# Method for timestepping. Can be RK222, RK443, CNAB2, MCNAB2, SBDF4
export STEPPER=RK222
# Aspect ratio in x and y resp.
export LX=2
export LY=2
# Provide the size of the square 2D process mesh; note that if your resolution is given by R,
# you have a grid of size Lx*R x Ly*R x R, and if your mesh is of size [n,m], then on each core you compute
# (Lx*R/n)x(Ly*R/m) pencils of length R. Most efficient when MESHX=MESHY. See Dedalus docs for more info.
export MESHX=16
export MESHY=24

# Use initial condition? (1=yes, 0=no)
export IC=1



# tmp_touchfile and parallel: see these places
export TMP=True
export PARA=mpio


# CFL Stuff
export CFL_SAFETY=0.75
export CFL_THRESHOLD=0.1
export CFL_CADENCE=5


# When to start averaging - post process
AVG_TIME=3200
# Exponent of 10 for minimum y axis on power spectra 
YMIN=-18
# Exponent of 10 for maximum y axis on power spectra 
YMAX=-3
# Exponent of 10 for minimum y axis on power spectra 
MINS="-18,-18,-18,-18"
MAXS="10,10,10,10"
################################################################################


# Path to all python scripts for simulations; change as needed
export PATH_TO_SCRIPTS="$SCRATCH/rbc_scripts"
export SCRIPTS_3D="$SCRATCH/rbc_scripts/3d"
export PATH_TO_ENV="$SCRATCH/ded3"

# Load the required modules
ml StdEnv/2023
ml python/3.11.5 mpi4py/4.0.3 fftw-mpi/3.3.10 scipy-stack/2023b ffmpeg/7.1.1
ml hdf5-mpi/1.14.4

source /scinet/vast/etc/vastpreload-openmpi.bash

source $PATH_TO_ENV/bin/activate;

# Dedalus performance tip!
export OMP_NUM_THREADS=1
export NUMEXPR_MAX_THREADS=1
export HDF5_USE_FILE_LOCKING='FALSE'


# Specify desired time for initial condition - 0 implies most recent
if [ $IC -eq 1 ]; then
  export IC_ARRAY=($(python3 $PATH_TO_SCRIPTS/initial_condition.py $START_TIME --file=$PWD/restart/restart.h5))
  export TOTAL_TIME=$(echo "$SIM_TIME+${IC_ARRAY[1]}" | bc)
  export IND=${IC_ARRAY[0]}
else
  echo "Not running with a prior simulation's initial conditions; starting fresh!"
  # rm -rf {analysis,state,preliminary_outputs,snapshots,outputs,field_analysis,restart}
  rm -rf restart
  export TOTAL_TIME=$SIM_TIME
  export IND=-1
fi

mpirun python3 $SCRIPTS_3D/rayleigh_benard_script.py --Ra=$RA --Pr=$PR --nz=$RES --dt=$DT --sim_time=$TOTAL_TIME --index=$IND --basepath=$PWD --stepper=$STEPPER --Lx=$LX --Ly=$LY --meshx=$MESHX --meshy=$MESHY --cfl_safety=$CFL_SAFETY --cfl_threshold=$CFL_THRESHOLD --cfl_cadence=$CFL_CADENCE --tmp=$TMP --para=$PARA --cfl


# For deciding the restart path
RECENT=$(find state/. -maxdepth 1 -type f -exec basename {} \; | sort -V | tail -n 1)

if [ ! -d "restart" ]; then
  mkdir restart
  echo "creating directory for restart"
fi

rm -rf restart/restart.h5
ln -sv $PWD/state/$RECENT $PWD/restart/restart.h5

python3 $PATH_TO_SCRIPTS/analysis_v3.py $PWD --time=$AVG_TIME

mkdir res_check
mkdir res_check_3d

mpirun python3 $PATH_TO_SCRIPTS/power_v3.py $PWD/state/*.h5 --ymin=$YMIN --ymax=$YMAX
mpirun python3 $SCRIPTS_3D/power_v3.py $PWD/state/*.h5 --mins=$MINS --maxs=$MAXS
ffmpeg -y -r 30 -pattern_type glob -i 'res_check/*.png' -threads 32 -pix_fmt yuv420p res_check/movie.mp4
ffmpeg -y -r 30 -pattern_type glob -i 'res_check_3d/*.png' -threads 32 -pix_fmt yuv420p res_check_3d/movie.mp4
