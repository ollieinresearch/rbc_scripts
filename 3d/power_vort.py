...
def main(file, start, count, mins, maxs):
    ...
    with h5.File(fp, 'r') as f:
        for ti in range(start, start+count):
            ...
            omega = np.array(f['tasks']['vorticity'][ti])

            nx, ny, nz = omega.shape       
            ...
            interp_omega = RegularGridInterpolator((x, y, z_cheb), omega, method='linear', bounds_error=False, fill_value=0)

            # Interpolate onto uniform z grid
            omega_uniform = interp_omega(pts).reshape((nx, ny, nz))
            ...
            del interp_omega, omega_uniform, Omega
            ...
            savename=f"{str(basepath)}/res_check_vort/write_{write:06}.png"
