...
#TODO: Redo this with new analysis file
...
def main(filename, start, count, start_ave,):
    ...
    output = Path("/project/def-goluskin/ollie/ollie_rb_data/3d/redoing_test")
    output.mkdir(parents=True, exist_ok=True)
    output = output / f"ra{ra}/pr{pr}/{res}_Gam2_2"
    output.mkdir(parents=True, exist_ok=True)

    # Rayleigh and Prandtl number from the folder structure
    Ra = float(ra)
    Pr = float(pr)
    ...
    with h5py.File(filename, mode="r") as file:        
        ...
        for ind, (qoi, nu_func, lab) in enumerate(qoi_func):
            ...
            dset = np.array(file["tasks"][qoi][:])[start_ind:, 0, 0, 0]
            ...
            # nth and n+1th indices, the start and end of each section.
            ...
            # Line to show final cumulative Nu
        ...
        dset_temp = np.array(file["tasks"]["avg_T"][:])[start_ind:, 0, 0, :]
        ...
        # end
        dset_K = np.array(file["tasks"]["h_avg_K"][:])[start_ind:, 0, 0, :]
        dset_u_sq = np.array(file["tasks"]["avg_u_sq"][:])[start_ind:, 0, 0, :]
        dset_w_sq = np.array(file["tasks"]["avg_w_sq"][:])[start_ind:, 0, 0, :]
...
if __name__ == "__main__":
    ...
    base_fp = Path("/project/def-goluskin/RB_data/3D/")
