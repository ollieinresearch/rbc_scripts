...
# All the exports were by recommendation of DRAC support when troubleshooting an issue
################################################################################
...
# (Lx*R/n)x(Ly*R/m) pencils of length R. Most efficient when MESHX=MESHY. See Dedalus docs for more info.
export MESHX=16
export MESHY=12
...
# tmp_touchfile and parallel: see these places
export TMP=True
export PARA=virtual


# CFL Stuff
export CFL_SAFETY=0.5
export CFL_THRESHOLD=0.1
export CFL_CADENCE=2
...
export PATH_TO_SCRIPTS="$SCRATCH/rbc_scripts"
export SCRIPTS_3D="$SCRATCH/rbc_scripts/3d"
export PATH_TO_ENV="$SCRATCH/ded3"
...
if [ $IC -eq 1 ]; then
  export IC_ARRAY=($(python3 $PATH_TO_SCRIPTS/initial_condition.py $START_TIME --file=$PWD/restart/restart.h5))
  export TOTAL_TIME=$(echo "$SIM_TIME+${IC_ARRAY[1]}" | bc)
  export IND=${IC_ARRAY[0]}
else
  ...
  rm -rf restart
  export TOTAL_TIME=$SIM_TIME
  export IND=-1
fi

mpirun python3 $SCRIPTS_3D/rayleigh_benard_script.py --Ra=$RA --Pr=$PR --nz=$RES --gamma=$GAM --dt=$DT --sim_time=$TOTAL_TIME --index=$IND --basepath=$PWD --stepper=$STEPPER --Lx=$LX --Ly=$LY --meshx=$MESHX --meshy=$MESHY --cfl_safety=$CFL_SAFETY --cfl_threshold=$CFL_THRESHOLD --cfl_cadence=$CFL_CADENCE --tmp=$TMP --para=$PARA --cfl
